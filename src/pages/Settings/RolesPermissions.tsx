...
export default function RolesPermissions() {
  ...
  const updatePermission = (roleId: string, moduleId: string, screen: string, permissionId: string, value: boolean) => {
  ...
  const hasPermission = (roleId: string, moduleId: string, screen: string, permissionId: string): boolean => {
    return permissionsMatrix[roleId]?.[moduleId]?.[`${screen}_${permissionId}`] || false;
  };

  // تحديد/إلغاء تحديد جميع الصلاحيات لشاشة معينة
  const toggleAllScreenPermissions = (roleId: string, moduleId: string, screen: string, value: boolean) => {
  ...
  // إضافة سجل تدقيق
  const addAuditLog = (action: string, target: string, details: string, type: 'role' | 'permission' | 'user') => {
  ...
  // تصدير البيانات
  const handleExport = (type: 'roles' | 'users' | 'permissions' | 'audit') => {
    let data: any[] = [];
    let filename = "";
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <Select 
                              ...
                              onValueChange={(value: "active" | "inactive") => setUserForm(prev => ({ ...prev, status: value }))}
                            >
                              <SelectTrigger className="mt-1">
                                <SelectValue />
                              </SelectTrigger>
