...
const PendingOrders = () => {
  ...
  const [newOrder, setNewOrder] = useState({
    ...
    buyerNumber: '',
    supplierNumber: '',
    status: 'initial' as Order['status'],
    totalPrice: 0,
    carrierNumber: '',
    draftInvoiceNumber: '',
    finalInvoiceNumber: '',
  ...
  const handleAddOrder = () => {
    ...
    toast({
      ...
      description: "تم إضافة الطلب الجديد إلى قائمة الطلبات المعلقة"
    });
  };
  ...
  const pendingCount = orders.filter(o => o.status === 'waiting_carrier' || o.status === 'initial').length;
  const totalValue = orders.reduce((sum, order) => sum + order.totalPrice, 0);

  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">الطلبات المعلقة</h1>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle>قائمة الطلبات المعلقة</CardTitle>
